...
namespace MyCollectionLibrary
{
    ...
    {
        public static IEnumerable<T> FilterByProperty<T, TProperty>(IEnumerable<T> items, Expression<Func<T, TProperty>> propertySelector, TProperty value)
        {
            var compiledSelector = propertySelector.Compile();
            return items.Where(item => EqualityComparer<TProperty>.Default.Equals(compiledSelector(item), value));
        }
    }
}
